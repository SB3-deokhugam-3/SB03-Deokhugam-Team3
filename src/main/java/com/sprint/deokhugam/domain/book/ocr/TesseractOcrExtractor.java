...
@Component
@Slf4j
@ConditionalOnProperty(name = "ocr.tesseract.enabled", havingValue = "true",matchIfMissing = false)
public class TesseractOcrExtractor implements OcrExtractor{

    // ISBN 패턴 정규식 (VisionOcrExtractor와 동일)
    ...
    public String extractIsbn(MultipartFile imageFile) throws OcrException {
        try {
            log.info("Tesseract를 사용하여 이미지에서 텍스트 추출 시작");
            ...
            log.info("Tesseract에서 추출된 텍스트 : {}", extractedText);
            ...
            log.error("Tesseract OCR 처리 실패", e);
    ...
    private String extractIsbnFromText(String text) {
        ...
        // 추가적인 ISBN 패턴 검색 ( 숫자만으로 구성된 10 자리 or 13자리 )
        ...
        while (numericMatcher.find()) {
            ...
            if (candidate.length() == 10 || candidate.length() == 13) {
                ...
                if (isValidIsbnFormat(candidate)) {
                    log.info("숫자 패턴에서 유효한 ISBN 발건 : {} ", candidate);
                    return candidate;
                }
            }
        }
    ...
    private boolean isValidIsbnFormat(String isbn) {
        // 기본적인 ISBN 형식 검증
        if (isbn.length() == 13) {
            // ISBN-13 978 또는 979로 시작해야 함
            return isbn.startsWith("978") || isbn.startsWith("979");
        } else if (isbn.length() == 10) {
            // ISBN-10 모든 문자가 숫자이거나 마지막 X일 수 있음
