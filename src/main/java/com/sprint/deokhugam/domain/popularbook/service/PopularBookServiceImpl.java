...
public class PopularBookServiceImpl implements PopularBookService {
     ...
     * 정렬 기준은 DESC(내림차순), ASC(오름차순)를 지원합니다. 커서 기반 페이지네이션 방식으로, cursor와 after 값을 기준으로 데이터를
     * 조회합니다.</p>
     *
     * @param request 인기 도서 조회 요청 객체 - period: 기간 유형 (예: DAILY, WEEKLY) - direction: 정렬 방향 (예: DESC)
     *                - cursor: 커서 기반 페이지네이션 기준 값 - after: 특정 시점 이후 데이터를 조회할 때 사용 - limit: 조회할 최대 도서
     *                수
